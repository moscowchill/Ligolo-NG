...
var (
	...
	commit  = "none"
	date    = "unknown"
	// Dummy variables to shift data layout
	dummyVar1 = "Lorem ipsum dolor sit amet, consectetur adipiscing elit"
	dummyVar2 = [256]byte{0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48}
	dummyVar3 = make([]string, 100)
)

func init() {
	// Use dummy vars to prevent optimization
	_ = dummyVar1
	_ = dummyVar2
	for i := range dummyVar3 {
		dummyVar3[i] = fmt.Sprintf("padding_%d", i)
	}
}
...
func main() {
	...
	var serverAddr = flag.String("connect", "", "connect to server (domain:port)")
	var bindAddr = flag.String("bind", "", "bind to ip:port")
	var userAgent = flag.String("ua", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "+
		"AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36", "HTTP User-Agent")
	var versionFlag = flag.Bool("version", false, "show the current version")

	flag.Usage = func() {
		fmt.Printf("NetAgent %s / %s / %s\n", version, commit, date)
		fmt.Println("Network utility tool")
		fmt.Println("https://github.com/user/netagent")
		fmt.Printf("\nUsage of %s:\n", os.Args[0])
		flag.PrintDefaults()
	}
